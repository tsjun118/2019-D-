...
inputfile='单GPS数据.xlsx'   #输入文件路径
#插值过程，采用样条插值，可有效避免拉格朗日插值出现的龙格现象
missingData = pd.read_excel(inputfile, header=None)   #将excel中数据读到dataframe，sheetname为sheet标号
...
for i in missingData.columns:
    for j in row:
        if missingData[i].isnull()[j]:            #得保证离上下界最近的空值<50
            ...
            missingData[i][j] =spi.splev(j,ipo3)  #试了几种方法，发现k=1为均值插值，比较适合数据，根据数据类型试验
            newrow +=[j]
            newcol +=[i]
missingData.to_excel('result2.xls', header=None, index=False)

#修改插值后单元格颜色过程
import xlwt
import xlrd
from xlutils.copy import copy
file_name = 'result2.xls' #与上面一致
n=len(newrow)
